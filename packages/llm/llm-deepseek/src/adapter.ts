...
class FileResolutionFailure extends Error {
  constructor(cause: unknown) {
    super('金石易服 Files API could not resolve a request image.', { cause })
...
function normalizedImageDiagnostic(
  ...
  if (target !== undefined) {
    return `金石易服 rejected normalized image ${normalizedImageFacts(target)}: ${providerMessage}. `
  ...
  return `金石易服 rejected a normalized request image: ${providerMessage}. Candidate images: `
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  private async * streamWithConnection(
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `金石易服 model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          '金石易服 image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (options.signal?.aborted) {
        throw new LlmError('金石易服 request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`DeepSeek API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('金石易服 stream consumer stopped')
  ...
  private async * request(
    ...
    while (true) {
      ...
      if (!response.body) {
        throw new LlmError('金石易服 API returned no response body', 'EMPTY_RESPONSE')
      }
