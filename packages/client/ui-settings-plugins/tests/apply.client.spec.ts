...
import type { ConfigurablePluginsTabFace } from '@deepseek-ai/dsh-client-ui-settings-plugins/client'
...
describe('ui-settings-plugins apply', () => {
  ...
  it('declares the tab and card slots', async () => {
  ...
  it('injects a live tab projection, the card directory, and one business face per card', async () => {
    ...
    expect(tabFace.hooks.configurablePlugins.getSnapshot().namespaces).toEqual([])

    const listener = vi.fn()
    const unsubscribe = slots.subscribe('settings.plugins.tab', listener)
